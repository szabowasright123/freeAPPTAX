...
import { limpiarFalloLectura, registrarFalloLectura } from './estadoDatos'
...
export function useLiveQuery<T>(
  ...
  useEffect(() => {
    const sub = liveQuery(consulta).subscribe({
      next: (datos) => {
        // Una lectura buena cierra la racha de fallos que pintaba el aviso de la cabecera.
        limpiarFalloLectura()
        setValor({ estado: 'listo', datos, error: undefined })
      },
      error: (err: unknown) => {
        // El aviso sube al sistema (data/estadoDatos.ts): las pantallas colapsan 'error' y
        // 'cargando' en un `: []` y, sin esto, un fallo de lectura se ve como Libro VACÍO.
        registrarFalloLectura(err)
        setValor({
          estado: 'error',
          datos: undefined,
