...
export function AsistenteEvento({
  ...
  const selUbic = (valor: string, onChange: (v: string) => void, etiqueta: string) => (
    <Field etiqueta={etiqueta}>
      <select className={INPUT_SISTEMA} value={valor} onChange={(e) => onChange(e.target.value)}>
  ...
  const tabla = (
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <span className="text-apoyo font-medium text-texto">{titulo}</span>
        <button
          type="button"
      ...
      {lista.map((t, i) => (
        <div key={i} className="grid grid-cols-3 gap-2">
          <input
            className={`${INPUT_SISTEMA} w-full`}
            placeholder="Activo"
            aria-label={`${titulo}: activo ${i + 1}`}
            value={t.activo}
          ...
          <input
            ...
            aria-label={`${titulo}: cantidad ${i + 1}`}
            value={t.cantidad}
          ...
          <input
            ...
            aria-label={`${titulo}: contravalor en euros ${i + 1}`}
    ...
    <Modal titulo="Nuevo evento DeFi" abierto={abierto} onCerrar={cerrar} ancho="max-w-3xl">
      {!clase ? (
        <div className="space-y-4">
          ...
          {FAMILIAS.map((f) => (
            <div key={f.titulo}>
              <h4 className="mb-1.5 text-caption font-semibold uppercase tracking-wide text-texto-mudo">
                {f.titulo}
              </h4>
              <div className="grid gap-2 sm:grid-cols-2">
                {f.opciones.map((o) => (
                  <Card key={o.clase} relleno="compacto" onClick={() => setClase(o.clase)}>
                    <span className="block text-cuerpo font-medium text-texto">{o.etiqueta}</span>
                    <span className="mt-0.5 block text-apoyo text-texto-secundario">
                      {o.ayuda}
                    </span>
                  </Card>
                ))}
        ...
        <div className="space-y-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-cuerpo font-medium text-texto">{opcion?.etiqueta}</p>
              <p className="text-apoyo text-texto-secundario">{opcion?.ayuda}</p>
            </div>
            <button type="button" className={BTN_SECUNDARIO} onClick={() => setClase(null)}>
              Cambiar
            </button>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            {clase === 'recompensa' && (
              <Field etiqueta="Origen del rendimiento">
                <select
                  className={INPUT_SISTEMA}
                  value={c.evento}
            ...
            {clase === 'hard-fork' && (
              <Field etiqueta="Postura (decisión manual)">
                <select
                  className={INPUT_SISTEMA}
                  value={c.postura}
            ...
            {clase === 'airdrop-condicionado' && (
              <label className="flex items-center gap-2 self-end text-cuerpo text-texto">
                <input
                  type="checkbox"
          ...
          <Field etiqueta="Criterio aplicado (se rellena solo en las zonas grises; puedes matizarlo)">
            <textarea
              className={`${INPUT_SISTEMA} h-16`}
          ...
          <Card relleno="compacto">
            <p className="mb-2 text-caption font-semibold uppercase tracking-wide text-texto-mudo">
              Apuntes que se crearán ({patas.length})
            </p>
            {patas.length === 0 ? (
              <p className="text-cuerpo text-texto-mudo">Completa los datos para ver el resultado.</p>
            ) : (
              <ul className="space-y-1.5 text-cuerpo text-texto">
                {patas.map((p, i) => (
                  <li key={i} className="flex flex-wrap items-baseline gap-x-2">
                    <Chip tono="brand">{ETIQUETA_TIPO[p.tipo]}</Chip>
                    {p.activoSalida && (
                    ...
                    {p.contravalorEUR && (
                      <span className="text-texto-secundario">({fmtEuro(p.contravalorEUR)})</span>
                    )}
                    {p.contravalorAlternativoEUR && (
                      <span
                        className="text-apoyo text-texto-mudo"
            ...
            {patas.length === 0 && clase === 'bloqueo' && c.ubicacionOrigen && c.ubicacionOrigen === c.ubicacionDestino && (
              <p className="mt-2 text-cuerpo text-texto-secundario">
          ...
          <div className="flex justify-end gap-2">
            <button type="button" className={BTN_SECUNDARIO} onClick={cerrar}>
              Cancelar
            </button>
            <button
              type="button"
