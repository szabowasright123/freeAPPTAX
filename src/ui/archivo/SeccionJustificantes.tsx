...
import { INPUT_SISTEMA, BTN_SECUNDARIO, Card, Chip, cx } from '../comp'
...
export function SeccionJustificantes({ tipo, conKyc, value, onChange }: Props) {
  ...
  return (
    <Card titulo="Justificantes (Archivo)">
      {/* Checklist probatoria del tipo */}
      <div className="mb-3 rounded-control bg-superficie p-2.5 text-apoyo">
        <p className="mb-1 font-medium text-texto-secundario">
          ...
          {requisitos.map((r) => {
            ...
            return (
              <li key={r.clave} className="flex items-start gap-1.5">
                <span aria-hidden className={ok ? 'text-semaforo-ok dark:text-green-400' : 'text-texto-mudo'}>
                  {ok ? '✓' : '○'}
                </span>
                <span className={ok ? 'text-texto-mudo line-through' : 'text-texto'}>
                  <span className="font-medium">{r.documento}</span>
                  {!ok && (
                    <button
                      type="button"
                      onClick={() => anadir(r.clave)}
                      className="ml-2 text-texto-acento underline underline-offset-2 hover:opacity-80"
                    >
                      añadir
                    </button>
                  )}
      ...
      <div className="space-y-2">
        {value.map((b, i) => (
          <div key={i} className="rounded-control border border-borde p-2.5">
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
              <label className="block text-apoyo">
                <span className="mb-0.5 block text-texto-secundario">Documento</span>
                <select
                  className={cx(INPUT_SISTEMA, 'w-full')}
              ...
              <label className="block text-apoyo">
                <span className="mb-0.5 block text-texto-secundario">Carpeta</span>
                <select
                  className={cx(INPUT_SISTEMA, 'w-full')}
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2 text-apoyo">
              {b.fichero ? (
                <Chip tono="ok">
                ...
                <label className={cx(BTN_SECUNDARIO, 'cursor-pointer')}>
                  Adjuntar fichero…
                  <input
                    type="file"
                    className="hidden"
              ...
              {!b.fichero && (
                <input
                  className={cx(INPUT_SISTEMA, 'flex-1')}
              ...
              <button
                type="button"
                className="ml-auto text-semaforo-error underline-offset-2 hover:underline dark:text-red-400"
                onClick={() => quitar(i)}
              >
                Quitar
              </button>
            ...
            {b.hashSHA256 && (
              <p className="mt-1 break-all font-mono text-caption text-texto-mudo" title="SHA-256">
      ...
      <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
        <button type="button" className={BTN_SECUNDARIO} onClick={() => anadir()}>
          + Añadir justificante
        </button>
        {espacioSesion > 0 && (
