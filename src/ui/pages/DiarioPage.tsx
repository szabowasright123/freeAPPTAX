...
declare module '@tanstack/react-table' {
  // Marca de columna numérica: `Tabla` la vuelca a `data-num` en el `<td>`/`<th>` y la alinea
  // a la derecha con `tabular-nums`. Las celdas las pinta TanStack, así que la señal viaja aquí.
...
/**
 * Reparto del ancho de la tabla, en porcentaje (suma 100). Con `table-fixed` estas anchuras
 * MANDAN sobre el contenido: la tabla cabe siempre en su contenedor y ninguna cabecera larga
 * empuja al resto; lo que no entra se recorta —el valor completo está en la ficha, a un clic—,
 * y como ninguna celda parte en dos líneas todas conservan la misma altura, de lo que depende
 * el virtualizador.
 *
 * Los porcentajes se MIDIERON sobre el caso de ejemplo con el relleno de `Tabla` (`px-3`, 24 px
 * por celda): cada columna de DATO cabe entera a `min-w` (68rem) y por tanto también a 1366 px.
 * Origen y destino se dimensionan para que el nombre CANÓNICO de una ubicación («Kraken»,
 * «Ledger», «EXTERIOR») quepa entero —«de dónde a dónde» se escanea tanto como el importe—; el
 * único recorte permitido es el de la ubicación que el alumno bautiza largo («Pool ETH/USDC
 * (Uniswap v3)»), que es el verdadero texto libre y lleva su `title`. Ese ancho se paga con la
 * holgura de `sello` (icono), `probatorio` (chip) y `salida`, no subiendo `min-w`. Si cambia el
 * relleno de `Tabla`, hay que volver a medir: los 8 px por celda salen de aquí.
 */
...
export function DiarioPage() {
  ...
  const columnas = useMemo(
    () => [
      col.accessor((f) => f.registro.id, {
        id: 'id',
        header: 'Nº',
        cell: (c) => <span className="font-mono text-apoyo text-texto-secundario">{c.getValue()}</span>,
      }),
      col.accessor((f) => f.registro.fechaHora, {
        id: 'fecha',
        header: 'Fecha',
        cell: (c) => <span className="text-texto-secundario">{fmtFechaHora(c.getValue())}</span>,
      }),
      ...
      col.accessor('origenKyc', {
        id: 'sello',
        // Columna estrecha: la cabecera es el icono del sello con su rótulo accesible en
        // sr-only, no un texto que se recortaría a «K..» en su 5 % de ancho.
        header: () => (
          <>
            <span aria-hidden="true" title="Origen KYC / no-KYC de la operación">
              {'\u{1F6E1}'}
            </span>
      ...
      col.accessor('salida', { header: 'Salida', meta: { num: true }, cell: (c) => <span title={c.getValue()}>{c.getValue()}</span> }),
      col.accessor('entrada', { header: 'Entrada', meta: { num: true }, cell: (c) => <span title={c.getValue()}>{c.getValue()}</span> }),
      col.accessor('comision', { header: 'Comisión', meta: { num: true }, cell: (c) => <span title={c.getValue()}>{c.getValue()}</span> }),
      col.accessor((f) => f.registro.contravalorEUR, {
        id: 'contravalor',
        header: 'Contravalor',
        meta: { num: true },
        cell: (c) => <span>{fmtEuro(c.getValue())}</span>,
      }),
  ...
  const virtualizador = useVirtualizer({
    ...
    // La altura de fila MANDA sobre el virtualizador: con la densidad `comoda` de `Tabla`
    // (`py-2.5` + `text-cuerpo`) la fila mide 43 px reales, medidos con los 500 apuntes del
    // generador. Si cambia el relleno de la tabla, este número cambia con él (contrato P8).
    estimateSize: () => 43,
    overscan: 12,
  })
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        ...
        subtitulo={
          <>
            ...
            {hayFiltros ? ` · ${filasFiltradas.length} tras filtro` : ''}. Numeración AAAA-NNN
            automática, orden cronológico.
          </>
        }
        acciones={
          <button
      ...
      {/* Plantillas y filtros en una barra compacta: antes eran bloques sueltos apilados
          sobre la tabla (§4). La ayuda de teclado ya no es un tercer bloque con separador:
          baja a una pista pegada a la tabla, más abajo. */}
      <Card relleno="compacto">
        <div className="space-y-2.5">
          {/* Plantillas rápidas: prerrellenan el formulario para operaciones frecuentes. */}
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-apoyo font-medium text-texto-mudo">Plantillas rápidas:</span>
            {/* Los eventos DeFi no caben en una plantilla de apunte: un solo hecho produce
                varias patas. Por eso van por asistente. */}
            <button type="button" className={BTN_SECUNDARIO} onClick={() => setAsistenteDefi(true)}>
              Evento DeFi…
            </button>
            {PLANTILLAS.map((p) => (
              <button
          ...
          {/* Buscador y filtros, en la misma barra que las plantillas y con poco aire. */}
          <div className="flex flex-wrap items-end gap-x-3 gap-y-2">
            <Field etiqueta="Buscar">
              <input
                className={`${INPUT_SISTEMA} w-56`}
            ...
            <Field etiqueta="Tipo">
              <select className={`${INPUT_SISTEMA} w-40`} value={fTipo} onChange={(e) => setFTipo(e.target.value)} aria-label="Filtrar por tipo">
            ...
            <Field etiqueta="Ubicación">
              <select className={`${INPUT_SISTEMA} w-40`} value={fUbic} onChange={(e) => setFUbic(e.target.value)} aria-label="Filtrar por ubicación">
            ...
            <Field etiqueta="Activo">
              <select className={`${INPUT_SISTEMA} w-32`} value={fActivo} onChange={(e) => setFActivo(e.target.value)} aria-label="Filtrar por activo">
                <option value="">Todos</option>
                {activos.map((a) => (
            ...
            <Field etiqueta="Año">
              <select className={`${INPUT_SISTEMA} w-28`} value={fAnio} onChange={(e) => setFAnio(e.target.value)} aria-label="Filtrar por año">
                <option value="">Todos</option>
                {anios.map((a) => (
            ...
            {hayFiltros && (
              <button type="button" className={BTN_SECUNDARIO} onClick={limpiarFiltros}>
                Limpiar
              </button>
            )}
          </div>
      ...
      {/* La tabla y, pegada encima como pista discreta, la ayuda de teclado (§4: ya no es un
          bloque con separador dentro de la barra). El contenedor de scroll es el de `Tabla`:
          no se anidan dos scrollers (contrato P8), y el `min-w` mantiene las columnas de dato
          legibles en móvil, desbordando DENTRO de la envoltura. */}
      <div className="space-y-1.5">
        <p className="px-1 text-apoyo text-texto-mudo">
        ...
        <Tabla
        className="table-fixed min-w-[68rem]"
        aria-label="Diario de apuntes"
        aria-rowcount={filasFiltradas.length}
        contenedorRef={contenedorRef}
        contenedorClassName="max-h-[70vh] overflow-y-auto"
        contenedorProps={{ onKeyDown: onTeclaTabla }}
      >
        <colgroup>
            ...
            <tr key={hg.id}>
              {hg.headers.map((h) => (
                <th
                  key={h.id}
                  scope="col"
                  data-num={h.column.columnDef.meta?.num ? '' : undefined}
        ...
        <tbody>
            ...
            return (
              <tr
                ...
                className={cx(
                  ...
                  // La fila activa es `superficie-acento` (el realce de marca del sistema) y su
                  // hover no la devuelve al fondo: gana en especificidad al hover de `Tabla`.
                  activa && 'bg-superficie-acento hover:bg-superficie-acento',
                )}
              >
                {row.getVisibleCells().map((cell) => (
                  <td
                    key={cell.id}
                    data-num={cell.column.columnDef.meta?.num ? '' : undefined}
                    className="truncate"
                  >
          ...
          {filasFiltradas.length === 0 && (
            <tr>
              <td colSpan={columnas.length} className="text-center text-texto-mudo">
    ...
    <Modal titulo={`Apunte ${r.id}`} abierto onCerrar={onCerrar}>
      <div className="space-y-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="tabular-nums text-texto-mudo">{fmtFechaHora(r.fechaHora)}</span>
        ...
        <dl className="grid grid-cols-[auto,1fr] gap-x-4 gap-y-1 text-cuerpo">
        ...
        <div>
          <h3 className="mb-1 text-caption font-semibold uppercase tracking-wide text-texto-mudo">Notas</h3>
          <p className="whitespace-pre-wrap break-words text-cuerpo">
            {r.notas?.trim() ? r.notas : <span className="text-texto-mudo">Sin notas.</span>}
          </p>
        </div>

        <div className="flex flex-wrap gap-2 border-t border-borde pt-3">
          <button type="button" className={BTN_SECUNDARIO} onClick={() => onEditar(r)}>
            Editar
          </button>
          <button type="button" className={BTN_SECUNDARIO} onClick={() => onDuplicar(r)}>
            Duplicar
          </button>
          <button
            type="button"
          ...
          <button type="button" className={`${BTN_PELIGRO_SISTEMA} ml-auto`} onClick={() => onBorrar(r)}>
            Borrar
          </button>
        </div>
      </div>
...
function Dato({ etiqueta, children }: { etiqueta: string; children: React.ReactNode }) {
  return (
    <>
      <dt className="text-texto-mudo">{etiqueta}</dt>
