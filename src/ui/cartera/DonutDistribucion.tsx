...
import { COLOR_OTROS } from './valoracion'
      ...
      <svg viewBox="0 0 144 144" className="mx-auto block h-40 w-40" role="img" aria-label="Distribución de la cartera por activo">
        {unico ? (
          <circle
            ...
            r={(R_EXT + R_INT) / 2}
            fill="none"
            stroke={segmentos.find((s) => s.valor > 0)?.color ?? COLOR_OTROS}
        ...
        {/* El tamaño va como atributo del SVG (geometría, escala con el viewBox), no como
            clase text-[Npx]; el color son los tokens de texto, que ya cambian con el tema. */}
        <text x={CX} y={CY - 4} textAnchor="middle" fontSize={8} className="fill-texto-mudo">
          Total
        </text>
        <text x={CX} y={CY + 8} textAnchor="middle" fontSize={10} className="fill-texto font-bold">
          {totalTexto}
        </text>
      </svg>

      {sel && (
        /* Panel invertido: `bg-texto` + `text-superficie` dan tooltip oscuro en claro y
           claro en oscuro sin escribir la pareja dark:. */
        <div className="pointer-events-none absolute inset-x-0 top-0 mx-auto w-max rounded-control bg-texto px-2 py-1 text-center text-caption text-superficie shadow-elevada">
