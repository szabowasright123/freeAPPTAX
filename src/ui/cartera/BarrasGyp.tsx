...
export function BarrasGyp({ datos }: { datos: GypEjercicio[] }) {
  ...
  return (
    <div className="overflow-x-auto">
      ...
      >
        {/* Eje cero (el hilo decorativo del sistema, que ya cambia con el tema). */}
        <line x1={0} y1={cero} x2={ancho} y2={cero} className="stroke-borde" strokeWidth={1} />
        ...
        {datos.map((d, i) => {
          ...
          return (
            <g key={d.ejercicio}>
              ...
              {/* Etiqueta directa con signo. El tamaño va como atributo del SVG (geometría
                  del dibujo, escala con el viewBox), no como clase text-[Npx]; el color son
                  los tokens de texto del sistema, que ya cambian con el tema. */}
              <text
                x={x + ANCHO_BARRA / 2}
                y={etiquetaY}
                textAnchor="middle"
                fontSize={10}
                className="fill-texto-secundario font-semibold"
              >
                {signo}
                {fmtEuro(d.netoEUR)}
              </text>
